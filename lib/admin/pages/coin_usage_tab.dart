...
class _CoinUsageTabState extends State<CoinUsageTab> {
  late final Stream<List<AdminStoryPack>> _packsStream = widget.storyRepository
      .watchPacks();
  late final Stream<List<AdminPackBundle>> _bundlesStream = widget
      .bundleRepository
      .watchAllBundles();
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<AdminStoryPack>>(
      ...
      builder: (context, packsSnapshot) {
        final packTitleById = {
          for (final p in packsSnapshot.data ?? const <AdminStoryPack>[])
            p.id: p.title,
        };
        return StreamBuilder<List<AdminPackBundle>>(
          ...
          builder: (context, bundlesSnapshot) {
            final bundleNameById = {
              for (final b in bundlesSnapshot.data ?? const <AdminPackBundle>[])
                b.id: b.name,
            };
  ...
  Widget _buildItemLabel(
    ...
    if (tx.isBundle) {
      ...
      final includedTitles = tx.bundlePackIds
          .map((id) => packTitleById[id] ?? id)
          .join(', ');
      return SizedBox(
        width: 220,
        child: Column(
